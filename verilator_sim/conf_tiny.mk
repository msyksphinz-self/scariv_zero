...
# RV32
#
rv32imc_tiny: $(FILELIST) .config_design_xlen32_flen$(RV_FLEN)
	$(MAKE) -C ../spike_dpi libspike_dpi.so VERILATOR=1
	$(MAKE) rv32_build CONF=tiny ISA=imc RV_XLEN=32 RV_FLEN=0 > $@_build.log 2>&1

rv32imfc_tiny: $(FILELIST) .config_design_xlen32_flen$(RV_FLEN)
	$(MAKE) -C ../spike_dpi libspike_dpi.so VERILATOR=1
	$(MAKE) rv32_build CONF=tiny ISA=imfc RV_XLEN=32 RV_FLEN=32 > $@_build.log 2>&1

rv32imfdc_tiny: $(FILELIST) .config_design_xlen32_flen$(RV_FLEN)
	$(MAKE) -C ../spike_dpi libspike_dpi.so VERILATOR=1
	$(MAKE) rv32_build CONF=tiny ISA=imfdc RV_XLEN=32 RV_FLEN=64 > $@_build.log 2>&1
...
# RV64
#
rv64imc_tiny: $(FILELIST) .config_design_xlen64_flen$(RV_FLEN)
	$(MAKE) -C ../spike_dpi libspike_dpi.so VERILATOR=1
	$(MAKE) rv64_build CONF=tiny ISA=imc RV_XLEN=64 RV_FLEN=0 > $@_build.log 2>&1

rv64imfc_tiny: $(FILELIST) .config_design_xlen64_flen$(RV_FLEN)
	$(MAKE) -C ../spike_dpi libspike_dpi.so VERILATOR=1
	$(MAKE) rv64_build CONF=tiny ISA=imfc RV_XLEN=64 RV_FLEN=32 > $@_build.log 2>&1

rv64imfdc_tiny: $(FILELIST) .config_design_xlen64_flen$(RV_FLEN)
	$(MAKE) -C ../spike_dpi libspike_dpi.so VERILATOR=1
	$(MAKE) rv64_build CONF=tiny ISA=imfdc RV_XLEN=64 RV_FLEN=64 > $@_build.log 2>&1
